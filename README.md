...
- 칸 번호는 각 셀의 직접 자식 `absolute` 요소로 두고 셀 내부 좌상단에 고정한다. 글자 크기는 8px이며 셀의 목표 배율이 아니라 매 프레임의 현재 spring scale에 맞춰 역스케일을 적용해 화면상 크기를 일정하게 유지한다. 좌상단 여백도 같은 현재 배율로 역보정하여 화면상 top 4px / left 5px를 유지한다. 지시문/플레이어 영역 사이의 구분선은 표시하지 않는다.
