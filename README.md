...
- 기본 이동 규칙은 경유 칸 지시문을 실행하지 않는 destination-only다. 최종 도착 칸 지시문 처리 완료 후에만 더블/윷/모 추가 던지기를 시작하며, 다음 던지기 스킵은 아직 실행하지 않은 보너스 던지기를 우선 취소한다.
- Bridge DB schema v7는 룸/플레이어 설정, 룸 생성 시점 방송 상태, 현재 보드 상태, 플레이어 위치, skip 상태, 처리된 `board.turn` 이벤트를 SQLite에 영속화하고 활성 보드게임 룸을 최대 1개로 제한한다.
