...
- 각 셀 내부는 방향과 크기에 관계없이 **지시문 영역 70% / 플레이어 영역 30%**로 고정 분할한다. 지시문은 보드 바깥쪽 영역, 플레이어 토큰은 보드 안쪽 영역에만 배치하며 셀이 확대되어도 비율은 변경하지 않는다.
...
- rounded 코너에서는 보드 안쪽 벡터의 우세 축을 기준으로 상/하/좌/우 지시문 영역을 선택한다. 같은 칸에 1~2명이 있을 때는 토큰 전체가 30% 플레이어 영역 안에 들어가도록 자동 축소·클램프하고, 3명 이상부터는 토큰 크기를 과도하게 줄이지 않고 지시문 반대편인 보드 안쪽 방향으로 셀 경계를 넘어갈 수 있게 배치한다.
