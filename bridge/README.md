...
1) +N / -N칸 이동           → 실제 도착 후 새 도착 칸 효과도 연쇄 실행
2) START로 이동             → START에 실제 도착, START는 아무 효과 없음
3) 다음 던지기 무효         → 가장 가까운 다음 실제 던지기 1회 소비
4) 다음 던지기 값 M배       → 2~100배, 다음 실제 던지기 이동값에 1회 적용
5) 다음 도착 칸 효과 무시   → 다음 실제 도착 칸 지시문을 1회 실행하지 않음
extra) 랜덤칸               → 수량(count)만 지정, 랜덤 후보 풀에서 최초/재선정
```

`다음 던지기 값 M배`는 주사위/윷의 원래 결과 자체를 바꾸지 않고 실제 이동값에 배율을 적용한다. 더블/윷/모 보너스 판정은 원래 결과를 기준으로 한다. `다음 던지기 무효`로 실제 던지기가 발생하지 않은 경우 대기 중 M배는 소비되지 않는다. `다음 도착 칸 효과 무시`는 경유 칸이 아니라 실제 도착 지시문에만 적용된다.
...
보드 렌더러는 각 셀 내부를 고정 비율로 분리한다. 보드 바깥쪽 **지시문 35%**, 안쪽 **플레이어 65%**이며 셀 확대나 다중 플레이어 점유에도 비율을 바꾸지 않는다. 토큰은 플레이어 영역 경계 안에서만 배치되고 필요한 경우 크기를 줄이므로 위쪽 칸을 포함해 지시문을 덮지 않는다.
