...
- 참가자별 이동 큐를 별도로 처리
- 화면 이동은 각 논리 한 칸마다 최소 한 번의 레이아웃 커밋을 보장해 rAF 지연으로 중간 칸이 합쳐져 보이지 않도록 한다. 마지막 스텝은 해당 플레이어 말의 스프링이 목표 위치에 정착한 뒤 완료로 처리하며, 과부하 시 제한 시간 후 목표 위치로 스냅해 다음 플레이어 이동 때 뒤늦게 따라가는 현상을 방지한다.
