...
보드 렌더러는 각 셀 내부를 고정 비율로 분리한다. 보드 바깥쪽 **지시문 70%**, 안쪽 **플레이어 30%**이며 셀 확대나 다중 플레이어 점유에도 비율을 바꾸지 않는다. `compact`는 시각 강조에 사용하지 않는다. 기본/인접/점유 상태만 테두리 강도와 배경 강조를 결정하며, 지시문 종류는 서로 다른 테두리 색으로 구분한다. 같은 칸의 플레이어가 1~2명일 때는 토큰 전체를 30% 플레이어 영역 안에 맞춘다. 3명 이상부터는 지시문 영역을 침범하는 대신 보드 안쪽 방향으로 셀 경계를 넘어갈 수 있게 2줄 배치한다.
