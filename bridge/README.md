...
보드게임 룸 설정은 전역 `config.json`과 분리하여 SQLite에 저장한다. DB schema v9는 룸/참가자 설정과 방송 상태, 보드 런타임 상태·플레이어 위치·처리 이벤트, 룸 수명주기, 일시정지 후원 큐와 방송 딜레이 유예 시각을 영속화하고 동시에 활성 가능한 보드게임 룸을 1개로 제한한다.
...
- 랜덤칸(`rerollOnVacate=true`)은 수량(`count`)으로만 지정 가능하며 비율 배치는 서버에서 거부
- 랜덤칸은 재선정 후에도 같은 위치가 계속 랜덤칸으로 유지
...
- 선택 정책 `IGNORE`는 유예 없이 일시정지 직후부터 일치 후원을 즉시 무시한다. 무시한 fingerprint도 기록해 재연결 재수신을 다시 처리하지 않는다.
- 재개 처리 중 Bridge가 종료되어 ACTIVE 상태에 잔여 큐가 남아도 시작 시 또는 다음 후원 전에 잔여 큐를 먼저 복구

기본 제공 칸 효과:

```text
0) 아무 효과 없음          → 별도 배치하지 않은 일반 칸
1) n칸 앞으로              → 실제 도착 후 새 도착 칸 효과도 연쇄 실행
2) n칸 뒤로                → 실제 도착 후 새 도착 칸 효과도 연쇄 실행
3) 다음 주사위 무효        → 가장 가까운 다음 실제 던지기 1회 소비
4) 다음 주사위 m배         → 2~100배, 다음 실제 던지기 이동값에 1회 적용
5) 다음 칸 무효화          → 다음 실제 도착 칸 지시문을 1회 실행하지 않음
extra) 랜덤칸              → 수량(count)만 지정, 랜덤 후보 풀에서 최초/재선정
```

`다음 주사위 m배`는 주사위/윷의 원래 결과 자체를 바꾸지 않고 실제 이동값에 배율을 적용한다. 더블/윷/모 보너스 판정은 원래 결과를 기준으로 한다. `다음 던지기 무효`로 실제 던지기가 발생하지 않은 경우 대기 중 M배는 소비되지 않는다. `다음 칸 무효화`는 경유 칸이 아니라 실제 도착 지시문에만 적용된다.

초기 보드 생성 및 `다시 배치` 시 서버는 랜덤칸을 제외한 고정칸 이동 지시문 그래프를 검사한다. `6 → 8 → 6`처럼 도착 지시문만으로 무한 순환하는 배치는 폐기하고 새 seed로 다시 생성하며, 확정 직전에도 같은 검사를 다시 수행한다. 랜덤칸에 진입하면 이후 결과가 비결정적이므로 고정 순환 검사 경로는 그 지점에서 종료한다.

현재 룰 기본값은 이동 중 경유 칸 지시문을 실행하지 않는 `destinationOnly`이다. 실제 도착 칸 지시문과 그 지시문으로 이어지는 새 도착 칸 효과를 모두 처리한 뒤 보너스 던지기를 진행하며, `다음 던지기 무효`는 아직 실행하지 않은 가장 가까운 던지기 1회를 소비하므로 더블/윷/모로 생긴 즉시 보너스 던지기도 취소할 수 있다.
...
도착 칸의 `+N/-N` 이동 지시문 때문에 다른 칸에 다시 도착하면 새 도착 칸의 지시문도 즉시 실행한다. 그 새 지시문이 다시 이동을 발생시키면 같은 규칙을 반복한다. 이동 중 경유한 칸은 여전히 실행하지 않는다. 잘못된 사용자 지시문 조합으로 무한 순환하는 것을 막기 위해 서버에는 landing-chain 안전 한도만 둔다.

보드 렌더러는 배치 계산상 보드 바깥쪽 **지시문 70%**, 안쪽 **플레이어 30%** 기준을 유지하지만 두 영역을 선·배경·별도 박스로 시각적으로 구분하지 않는다. `compact`는 시각 강조에 사용하지 않고 기본/인접/점유 상태만 테두리 강도와 배경 강조를 결정하며, 지시문 종류는 서로 다른 테두리 색으로 구분한다. 플레이어 토큰 크기는 70/30 비율 때문에 줄이지 않고 기존 셀 크기 기준 공식을 유지한다. 같은 칸의 플레이어가 1~2명일 때는 전체 토큰을 셀 안에 유지하고, 3명 이상부터는 보드 안쪽 방향으로 셀 경계를 넘어갈 수 있게 2줄 배치한다. 칸 번호는 셀 확대/축소와 무관하게 화면상 고정 크기를 유지한다.
