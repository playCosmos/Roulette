...
보드게임 룸 설정은 전역 `config.json`과 분리하여 SQLite에 저장한다. DB schema v8는 룸/참가자 설정과 방송 상태, 보드 런타임 상태·플레이어 위치·처리 이벤트, 룸 수명주기, 일시정지 후원 큐를 영속화하고 동시에 활성 가능한 보드게임 룸을 1개로 제한한다.
...
- 일시정지 기본 후원 정책은 `QUEUE`: 정확 trigger 후원을 SQLite FIFO 큐에 누적하고 재개 시 수신 순서대로 처리
- 선택 정책 `IGNORE`: 일시정지 중 일치 후원을 처리하지 않으며 fingerprint를 기록해 재연결 재수신도 다시 처리하지 않음
