...
- 참가자별 이동 큐를 별도로 처리
- 화면 이동은 레이아웃 완료 Promise나 `waitForLayoutCommit`에 의존하지 않는다. **보드 셀의 Dock 재배치는 기존 critically-damped spring과 원래 P0 정밀도를 유지**하고, 이동 중인 플레이어 말만 별도의 한 칸 단위 애니메이션으로 분리한다. 각 논리 스텝에서 보드 레이아웃을 rAF로 예약한 뒤 최소 한 프레임을 넘기고, 말은 이전 실제 위치에서 해당 칸의 최신 목표 위치까지 `STEP_DELAY_MS` 동안 반드시 완료한다. 다른 플레이어 때문에 목표가 움직여도 매 프레임 최신 목표를 추적하며, rAF 지연 시에도 제한 시간 후 최신 목표로 확정해 무한대기를 방지한다. 모든 셀/다른 플레이어 말을 강제로 스냅하지 않는다.
