...
- 각 셀은 플레이어 점유 시 방향과 관계없이 두 영역을 선·배경·별도 박스로 시각적으로 나누지 않고 **지시문 50% / 플레이어 50%**를 사용한다. 위/아래 가로칸과 좌/우 세로칸 모두 동일한 비율을 사용한다. 지시문 칸과 START는 비점유 시 텍스트를 셀 전체 중앙에 더 크게 표시하고, 플레이어가 도착하면 해당 방향의 점유 비율에 맞춰 지시문을 바깥쪽으로 이동한다.
