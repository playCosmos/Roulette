...
- 관리자 일시정지는 기본적으로 후원을 FIFO 큐에 누적하며 재개 시 오래된 후원부터 처리한다. 생성/일시정지 시 `IGNORE`를 선택하면 정지 중 후원을 기록 후 무시한다.
...
- Bridge DB schema v8는 룸/플레이어 설정, 방송 상태, 현재 보드 상태, 플레이어 위치, 1회성 효과 상태, 처리된 `board.turn` 이벤트, 룸 수명주기와 일시정지 후원 큐를 SQLite에 영속화하고 활성 보드게임 룸을 최대 1개로 제한한다.
