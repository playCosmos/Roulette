...
- 참가자별 이동 큐를 별도로 처리
- 화면 이동은 레이아웃 완료 Promise나 `waitForLayoutCommit`에 의존하지 않는다. 각 논리 한 칸마다 레이아웃을 즉시 계산해 토큰 목표 좌표를 확정하고, 최소 한 번의 실제 페인트 기회와 스텝 시간을 보장한 뒤 해당 칸 목표 좌표에 정확히 착지시킨 다음 다음 칸으로 진행한다. 레이아웃 계산이 실패하면 다음 일반 프레임에서 재시도하되 이동 큐는 대기시키지 않아 교착을 방지한다. 따라서 중간 눈금이 합쳐져 덜 이동해 보이거나 다른 플레이어 이동 때 뒤늦게 따라가는 현상을 방지한다.
