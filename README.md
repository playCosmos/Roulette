...
- 데모 플레이어 토큰에는 대기 중인 1회성 효과를 작은 배지로 표시한다. `무효`, `×n`, `칸 무효`를 사용하며 효과가 소비되는 즉시 제거한다. 배지는 지시문 반대편인 보드 안쪽 플레이어 영역에 배치하고, 결과 말풍선이 동시에 표시되면 말풍선을 그보다 더 안쪽으로 밀어 지시문을 가리지 않는다.
