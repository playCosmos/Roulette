...
보드 렌더러는 각 셀 내부를 고정 비율로 분리한다. 보드 바깥쪽 **지시문 35%**, 안쪽 **플레이어 65%**이며 셀 확대나 다중 플레이어 점유에도 비율을 바꾸지 않는다. `compact`는 시각 강조에 사용하지 않는다. 기본/인접/점유 상태만 테두리 강도와 배경 강조를 결정하며, 지시문 종류는 서로 다른 테두리 색으로 구분한다. 토큰은 플레이어 영역 경계 안에서만 배치되고 필요한 경우 크기를 줄이므로 위쪽 칸을 포함해 지시문을 덮지 않는다.
