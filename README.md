...
- 참가자별 이동 큐를 별도로 처리
- 보드 레이아웃/Dock 계산과 셀 스프링 구현은 변경하지 않는다. 기존 V34/V35/V3/V4 이동 구현은 회귀 비교용으로 보존하고, active 이동은 V5를 사용한다. 목적지는 논리 위치와 이동량으로 항상 결정한다. 각 스텝에서 `layoutNow()` 재계산이 예외를 내더라도 **이동 루프를 중단하거나 이동량을 줄이지 않는다**. 정상 계산이면 새 token target을 사용하고, 예외 시에는 마지막 정상 레이아웃의 목적지 셀 현재 화면 geometry로 토큰 위치를 계산해 해당 칸까지 이동한 뒤 보드 레이아웃만 다음 rAF에서 재시도한다. 레이아웃 오류는 게임 상태 전이를 취소하지 않는다.
...
- 각 셀은 플레이어 점유 시 방향에 따라 공간 비율을 다르게 사용하되 두 영역을 선·배경·별도 박스로 시각적으로 나누지 않는다. 위/아래 가로칸은 **지시문 50% / 플레이어 50%**, 좌/우 세로칸은 **지시문 70% / 플레이어 30%**를 사용한다. 지시문 칸과 START는 비점유 시 텍스트를 셀 전체 중앙에 더 크게 표시하고, 플레이어가 도착하면 해당 방향의 점유 비율에 맞춰 지시문을 바깥쪽으로 이동한다.
- 플레이어 말은 지시문/플레이어 영역 경계에 바로 붙이지 않고 약 5px의 안쪽 여유를 둔다. 말 크기는 변경하지 않으며 1~2명 내부 배치와 3명 이상 안쪽 확장 모두 같은 기준을 사용한다.
