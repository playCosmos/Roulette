...
- 각 셀은 플레이어 점유 시 **지시문 70% / 플레이어 30%** 기준을 사용하지만 두 영역을 선·배경·별도 박스로 시각적으로 나누지 않는다. 지시문 칸과 START는 비점유 시 텍스트를 셀 전체 중앙에 더 크게 표시하고, 플레이어가 도착하면 텍스트를 바깥쪽 70% 영역으로 이동해 안쪽 30%를 플레이어 공간으로 확보한다.
