...
- 칸 번호 제거 후 지시문 라벨은 번호용 좌상단 여백을 예약하지 않는다. 상/하 셀은 좌우 대칭 패딩과 최대 2줄을 사용한다. 좌/우 세로 셀은 비점유 시 중앙의 큰 텍스트를 유지하지만, 플레이어가 도착한 점유 상태에서는 지시문과 START 모두 한 줄 최대 3글자를 기준으로 최대 3줄까지 강제 줄바꿈한다. 예: `다음 / 던지기 / 무효`.
