...
- 각 셀 내부는 방향과 크기에 관계없이 **지시문 영역 35% / 플레이어 영역 65%**로 고정 분할한다. 지시문은 보드 바깥쪽 영역, 플레이어 토큰은 보드 안쪽 영역에만 배치하며 셀이 확대되어도 비율은 변경하지 않는다.
...
- rounded 코너에서는 보드 안쪽 벡터의 우세 축을 기준으로 상/하/좌/우 지시문 영역을 선택하고, 토큰 크기와 다중 플레이어 배치는 65% 플레이어 영역을 넘지 않도록 자동 축소·클램프한다.
