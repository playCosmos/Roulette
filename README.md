...
- v2 레이아웃은 상·우·하·좌 4개 Flex 레인으로 분리하고 CSS `gap`을 단일 간격 기준으로 사용하여 셀 크기 변화 중에도 edge-to-edge 간격을 유지
- 참가자 강조 영향은 여러 참가자의 값을 누적하지 않고 각 칸의 최대 국소 영향값을 사용하여 참가자 수 증가로 보드 전체가 함께 팽창하는 현상을 방지
- 기존 좌표 기반 구현은 `games/board/js/board.js`, `games/board/css/board.css`와 `games/board/legacy.html`로 보존하고, v2는 별도 `board-v2.js`/`board-v2.css`에서 운용
- 보드게임 참가자는 최대 6명으로 제한하며, 참가자 수에 따른 별도 전역 감쇠/고밀도 보정 없이 가장 가까운 참가자의 국소 확대 영향만 적용
