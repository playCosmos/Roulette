...
- 참가자별 이동 큐를 별도로 처리
- 화면 이동은 레이아웃 완료 Promise나 `waitForLayoutCommit`에 의존하지 않는다. 각 논리 한 칸마다 레이아웃을 즉시 계산해 토큰 목표 좌표를 확정하고, 최소 한 번의 실제 페인트 기회와 스텝 시간을 보장한 뒤 현재 스텝의 **셀 재배치와 플레이어 말 모두를 목표 상태에 확정**하고 다음 칸으로 진행한다. 이전 스텝의 셀 스프링뿐 아니라 **모든 플레이어 말의 재배치 스프링도 해당 스텝 안에서 목표 위치에 확정**해 다음 플레이어 이동 때 다른 말이 뒤늦게 따라가는 현상을 막는다. 레이아웃 계산이 실패하면 다음 일반 프레임에서 재시도하되 이동 큐는 대기시키지 않아 교착을 방지한다. 또한 P0의 0.05px gap 허용오차를 만족하는 범위에서 내부 이분 탐색 반복 수를 줄여 이동 중 동기 solver로 인한 프레임 정지를 최소화한다.
...
- 데모 모드는 START를 제외한 칸 중 약 28%(최대 15칸)를 임의 선택해 `n칸 앞으로`, `n칸 뒤로`, `다음 던지기 무효`, `다음 던지기 m배`, `다음 칸 무효화` 5종을 순환 배치하며, n과 m은 데모 시작 시 임의 값으로 정한다. 데모에서도 이동 지시문 연쇄, 다음 던지기 무효, 다음 실제 던지기 1회 배율, 다음 칸 효과 무효화를 실제로 실행한다. 보너스 던지기가 대기 중이면 `다음 던지기 무효`가 그 보너스를 먼저 소비하며, 이동 지시문 순환은 안전 제한으로 중단한다.
