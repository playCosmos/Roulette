...
실제 서버 PC에서 `http://127.0.0.1:17830/`을 열면 룸 운영 화면이 아니라 서버 자체 관리 콘솔을 표시한다.

표시 항목:
- 활성 원격 관리자 인증 세션 수
- 현재 WebSocket 연결 수
- SOOP 연결 상태와 Streamer ID
- 서버 버전, 인스턴스 ID, 시작 시각/가동 시간
- 17830/17832/17831 엔드포인트
- DB 경로
...
- 사용자 관리자 열기: 서버 PC에서 17832 관리자 UI를 새 창으로 연다.

17830은 계속 `127.0.0.1` 로컬 전용이며 포트포워딩 대상이 아니다. 게임 운영/룸 제어는 17832 관리자 UI에서 수행한다.
