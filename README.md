...
- 기본 제공 칸은 `아무 효과 없음`, `+N/-N칸 이동`, `START로 이동`, `다음 던지기 무효`, `다음 던지기 값 M배`, `다음 도착 칸 효과 무시`, 그리고 랜덤칸이다.
- M배는 2~100 범위이며 다음 실제 던지기의 이동값에 1회 적용한다. 주사위 더블/윷/모 여부는 배율 적용 전 원래 결과로 판정한다.
- 다음 도착 칸 효과 무시는 실제 도착 지시문 1회만 소비하며, 그 칸의 이동/START/스킵/배율 효과는 실행하지 않는다.
