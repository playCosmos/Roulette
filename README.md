...
- 각 셀은 플레이어 점유 시 **지시문 70% / 플레이어 30%** 기준을 사용하지만 두 영역을 선·배경·별도 박스로 시각적으로 나누지 않는다. 일반 지시문 칸은 비점유 시 지시문을 셀 전체 중앙에 더 크게 표시하고, 플레이어가 도착하면 지시문을 바깥쪽 70% 영역으로 이동해 안쪽 30%를 플레이어 공간으로 확보한다. START는 점유 여부와 무관하게 항상 셀 중앙에 표시한다.
...
- 칸 번호 제거 후 지시문 라벨은 번호용 좌상단 여백을 예약하지 않는다. 상/하 셀은 좌우 대칭 패딩과 최대 2줄을 사용한다. 좌/우 세로 셀은 비점유 시 중앙의 큰 지시문을 유지하지만, 플레이어가 도착한 점유 상태에서는 한 줄 최대 3글자를 기준으로 최대 3줄까지 강제 줄바꿈한다. 예: `다음 / 던지기 / 무효`.
