...
- 칸 번호는 각 셀의 직접 자식 `absolute` 요소로 두고 셀 내부 좌상단에 고정한다. 글자 크기는 8px이며 셀의 목표 배율이 아니라 매 프레임의 현재 spring scale에 맞춰 역스케일을 적용해 화면상 크기를 일정하게 유지한다. 번호의 위치와 역스케일은 하나의 `transform`에서 동시에 갱신하며 `top/left`는 프레임마다 변경하지 않는다. 기본 화면상 여백은 top 4px / left 5px이고, 확대된 셀에서는 현재 화면상 `border-radius`의 45도 코너 안전 inset을 반영해 둥근 모서리와의 간격을 연속적으로 늘린다. 지시문/플레이어 영역 사이의 구분선은 표시하지 않는다.
